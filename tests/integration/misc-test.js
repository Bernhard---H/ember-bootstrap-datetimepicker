...
module('ember-bootstrap-datetimepicker integration', function(hooks) {
  ...
  test("test placeholder support", async function(assert) {
    ...
    assert.dom('input').hasAttribute('placeholder', 'test-placeholder');
  });

  test("test the inline option", async function(assert) {
    assert.expect(2);

    assert.equal($(".bootstrap-datetimepicker-widget").length, 0, "no date picker before render");

    await render(hbs`{{bs-datetimepicker inline=true}}`);
